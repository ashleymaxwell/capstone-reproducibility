...
str(dat)
```
As we can see dat is a data frame with two variables:music and words per minute
...
```
The mean values show that the metal group can type more words per minute

Now I'll perform an independent t-test to see if my intuition was correct
...
t.test(dat$wpm[dat$music=="none"],
       ...
       var.equal=TRUE,)
```
From the results of our t-test we can see that the metal group types more words per minute than the control group, t(62)=-2.33, p=.0229

Now I will run another independent t-test with the alternative hypothesis that the metal group types more words per minute. 
<!--
...
t.test(dat$wpm[dat$music=="none"],
       ...
       alternative="greater")
```
From the results we can see that the metal group types more words per minute than the control group, t(62)=-2.33, p=.9885
...
-->
Now I will conduct a  two tailed paired t-test on the data.
```{r}
t.test(dat$wpm[dat$music=="none"],
       dat$wpm[dat$music=="metal"],
       var.equal=TRUE,
       paired=TRUE)
```
From my analysis, Individuals in the metal group typed more WPM than the group that had no music, 
t(31)=-12.15, p<.05
